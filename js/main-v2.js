...
//----------------------------------------------------------
// equip filter
const lan_eq_type = Object.keys(parsetype).map(
    key => (key = Object.assign({ id: key }, parsetype[key]))
  ),
  ...
  value:
    ...
    function (arr) {
      if (!(arr instanceof Array))
        throw Error(`[${arr}] (${typeof arr}) is not Array!!`)
      return JSON.stringify(this) === JSON.stringify(arr)
    }
})

//----------------------------------------------------------
// notice manager helper object

const noticeManager = {
  checkAndShowNotice () {
    const dismissed = LS.userSetting.get(settingKey.noticeDismissed)
    const noticeElement = document.getElementById('site_notice')
  ...
  addDismissHandler () {
    const noticeElement = document.getElementById('site_notice')
    const closeBtn = noticeElement.querySelector('.close-notice')

    if (closeBtn) {
      closeBtn.addEventListener('click', () => {
        noticeElement.style.display = 'none'
        LS.userSetting.set(settingKey.noticeDismissed, '1')
      })
    }
  }
}
...
const settingKey = {
  ...
  util = {
    ...
    sorting (arr, key, descen) {
      if (descen) {
        arr.sort((a, b) => {
          return naturalSort(b[key], a[key])
        })
      } else {
        arr.sort((a, b) => {
  ...
  LS = {
    ...
    fleetManager: {
      ...
      storageFleetData (fleet_data = []) {
        ...
        if (!(fleet_data instanceof Array))
          throw Error('fleet data is not Array')
    ...
    async exportUserData () {
      ...
      console.log(data_ls)
      download(
        `AzurLane_Fleet_Tool_${new Date().toISOString()}.alft`,
        data_export
      )
    ...
    async importUserData (addMode) {
      ...
      if (ck != CryptoJS.MD5(data_json).toString())
      ...
      async function loadData () {
        for (let key of Object.keys(data_ls)) {
          if (key != '_fleets_') {
            ...
            if (!addMode) {
              LS.fleetManager.clearFleetData()
              if (data_ls[key].length > 0)
                LS.fleetManager.storageFleetData(data_ls[key])
            } else {
              if (data_ls[key].length > 0)
                LS.fleetManager.addFleetData(data_ls[key])
            }
          }
        }
        return true
  ...
  msg = {
    error: {
      _t (text = '') {
        return fleet_info.msg.red('Error: ' + text)
      },
      delete_last () {
        return this._t("You can't delete the last Fleet")
      },
      maximum_fleet () {
        return this._t('Exceed maximum Fleet limit')
      },
      negative_position () {
        return this._t('Position must be positive')
      },
      unknown_direction () {
        return this._t('Unknown direction')
      },
      empty_name () {
        return this._t('Fleet name is empty')
      },
      invalid_id () {
        return this._t('Invalid Fleet id')
      },
      no_data () {
        return this._t('Fleet data is empty')
      },
      long_url () {
        return this._t(
          'URL too long. You still can share it by use fleetdata below'
        )
      },
      too_high () {
        return this._t("Can't yeet the Fleet higher than it is")
      },
      too_low () {
        return this._t("Can't yeet the Fleet lower than it is")
      },
      unzip_failed () {
        return this._t('Invalid data')
      },
      corrupted_data (t) {
        return this._t(`Corrupted data` + (t.length ? ` (${t})` : ''))
      },
      unknown_version (t) {
        return this._t('Unknown version' + (t.length ? ` (${t})` : ''))
      },
      wrong_data_type (t) {
        return this._t(`Wrong data type` + (t.length ? ` (${t})` : ''))
      }
    },
    normal: {
      _t (text = '') {
        return fleet_info.msg.green(text)
      },
      storage_add_fleet (name) {
        return this._t(`Fleet [${decodeURIComponent(name)}] Saved`)
      },
      storage_load_fleet (fleet_id, name) {
        return this._t(`Fleet ${fleet_id} [${decodeURIComponent(name)}] loaded`)
      },
      storage_remove_fleet (fleet_id, name) {
        return this._t(
          `Fleet ${fleet_id} [${decodeURIComponent(name)}] removed`
        )
      },
      storage_found_fleets (num) {
        return this._t(`Found ${num} Fleet in storage`)
      },
      copied () {
        return this._t('Copied')
      },
      fleet_dump () {
        return this._t(`Fleet data dumped`)
      },
      fleet_copied (fleet_id) {
        return this._t(`Fleet ${fleet_id + 1} copied`)
      },
      fleet_removed (fleet_id) {
        return this._t(`Fleet ${fleet_id + 1} removed`)
      },
      fleet_added (type) {
        return this._t(`New ${type == 1 ? 'Normal' : 'Submarine'} Fleet added`)
      },
      fleet_loaded () {
        return this._t('Successfully loaded Fleet data')
      },
      owned_dump (s_count, e_count) {
        return this._t(`Owned data dumped [Ship:${s_count}, Equip:${e_count}]`)
      },
      owned_load (s_count, e_count) {
        return this._t(
          `Owned data loaded (text) [Ship:${s_count}, Equip:${e_count}]`
        )
      },
      owned_load_setting (s_count, e_count) {
        return this._t(
          `Owned data loaded (setting) [Ship:${s_count}, Equip:${e_count}]`
        )
      },
      owned_save (s_count, e_count) {
        return this._t(`Owned data saved [Ship:${s_count}, Equip:${e_count}]`)
      },
      ship_copied (ship) {
        return this._t(`[${ship[0].property[language]}] copied`)
      },
      device_width () {
        return this._t(
          `Layout is set to [ ${layout_list.v2} ] due to undersized screen.`
        )
      }
    }
  },
  fleet_info = {
    ...
    msg: {
      ...
      clear_queue () {
        this.queue.forEach(tid => clearTimeout(tid))
      },
      queue: []
    }
  },
  classManager = {
    ...
    batchExchange (ele, class_1, class_2) {
      if (
        (class_1 && class_2) instanceof Array &&
        (class_1 && class_2).length > 0
      ) {
  ...
  app = {
    option: {
      setLanguage (ele) {
        ...
        document
          ...
          .forEach(name => (name.textContent = name.getAttribute(key)))
        document.querySelectorAll("[ui_text='true']").forEach(ui_ele => {
          if (ui_ele.id.includes('affinity_')) {
            ui_ele.parentElement.childNodes[1].textContent =
              ui_ele.getAttribute(`ui_${key}`)
          } else if (ui_ele.type == 'radio') {
            ui_ele.parentElement.childNodes[2].textContent =
              ui_ele.getAttribute(`ui_${key}`)
          } else {
      ...
      setInfo (ele) {
        ...
        if (ele instanceof HTMLElement)
          LS.userSetting.set(settingKey[key], is_active)
      },
      ...
      compactMode (ele, index) {
        ...
        if (is_active) {
          ...
          if (this._last_compact_mode != undefined)
        ...
        if (ele instanceof HTMLElement)
          LS.userSetting.set(settingKey.compactMode, is_active ? ++index : 0)
        eles.forEach(_ele => {
          if (_ele.id != ele.id) {
          ...
          for (let target in appClassData) {
            if (target == 'app_box') {
              ...
              document.querySelector(
                '.app_box'
              ).className = `${appClassData.app_box[layout]} app_box`
            } else {
      ...
      displayBorder (ele) {
        $(ele).button('toggle')
        let layoutKey =
            layout_list[document.querySelector('#layout_setting').textContent],
        ...
        if (display) {
          for (let target in list) {
            ...
            layout_list.forEach(layout => {
              appClassData[target][layout] = class_data[layout]
                .split(' ')
                .concat(border_class)
                .join(' ')
              if (layout == ALF.class_data.current)
                ALF.class_data[target] = appClassData[target][layout]
            })
          }
        } else {
      ...
      adjustEle () {
        ...
        if (width < safe_size) {
          target_list.forEach(t => {
            t = converter(...t)
            classManager[t.mode](t.ele, t.n, t.s)
          })
        } else {
          target_list.forEach(t => {
            t = converter(...t)
            classManager[t.mode](t.ele, t.s, t.n)
          })
        }
      },
      ship: {
        allow_dup (ele) {
          ...
          LS.userSetting.set(
            settingKey.allowDup,
            ele.classList.contains('active') ? 1 : 0
          )
        },
        setCode (ele) {
          $(ele).button('toggle')
        ...
        sort_order (ele) {
          ...
          // trigger sort
          document
            .querySelector(`[onclick*="app.option.ship.sort"]`) //ship
            .parentElement.querySelector(
              `[aria-labelledby="sort_menu"]>.active`
            )
            .click()
        },
        isDescend () {
          if (!this._sort_order)
            this._sort_order = document.querySelector('#ship_sort_order')
        ...
        sort (ele, reset = false) {
          let shiplist = document.querySelector('#shiplist'),
            ...
            top = sortedShip[0],
            list = (
              this.last_sort.list ? this.last_sort.list : sortedShip
            ).filter(ship => ship.id != tid),
          ...
          list.forEach(ship =>
            shiplist.appendChild(shiplist.querySelector(`[id="${ship.id}"]`))
          )
          this.last_sort = { key, descend, list }
          // clear other selected
          ;[...ele.parentElement.children].forEach(item =>
      ...
      equip: {
        resetFilter (toDefault = false) {
          ...
          document
            .querySelectorAll('#eq_tier button')
            .forEach(b =>
              util.changeButtonStats(
                b,
                default_tier.has(parseInt(b.value, 10)) ? true : false
              )
            )
          document
            .querySelectorAll('#eq_rarity button')
            .forEach(b =>
              util.changeButtonStats(
                b,
                default_rarity.has(parseInt(b.value, 10)) ? true : false
              )
            )
          document
            .querySelectorAll('#eq_nation button,#eq_type button')
            .forEach(b => util.changeButtonStats(b, false))
        ...
        sort_order (ele) {
          ...
          // trigger sort
          document
            .querySelector(`[onclick*="app.option.equip.sort"]`) //equip
            .parentElement.querySelector(
              `[aria-labelledby="sort_menu"]>.active`
            )
            .click()
        ...
        isDescend () {
          if (!this._sort_order)
            this._sort_order = document.querySelector('#equip_sort_order')
          return this._sort_order.value == 1 ? true : false
        },
        last_sort: {},
        sort (ele, reset = false) {
          let equiplist = document.querySelector('#equiplist'),
            tid = '666666',
            top = sortedEquip[0],
            list = (
              this.last_sort.list ? this.last_sort.list : sortedEquip
            ).filter(equip => equip.id != tid),
            key = ele.getAttribute('value'),
            descend = this.isDescend()
          console.log(`sort by:${key}, descend:${descend}`)
          if (!reset) {
          ...
          list.forEach(equip =>
            equiplist.appendChild(equiplist.querySelector(`[id="${equip.id}"]`))
          )
          this.last_sort = { key, descend, list }
          // clear other selected
          ;[...ele.parentElement.children].forEach(item =>
    ...
    fleet: {
      buildFleet (formation_data = [], update = false) {
        ...
        formation_data.forEach((formation, fleet_id) =>
          newfleet.push(formationSel[formation](fleet_id))
        )
        ...
        for (let position = 0; position < 6; position++) {
          if (position < 3) {
            ...
            for (let itemIndex = 0; itemIndex < max; itemIndex++) {
              ...
              property.pos = 'front'
              ship.push({
                id: `${fleetID}_0_${position}_${itemIndex}`,
                property: property
              })
            }
            front.push({
              id: `fleet_${fleetID}_front_${position}`,
              item: ship
            })
          } else {
            let ship = []
            for (let itemIndex = 0; itemIndex < max; itemIndex++) {
              ...
              property.pos = 'back'
              ship.push({
                id: `${fleetID}_1_${position - 3}_${itemIndex}`,
                property: property
              })
            }
            back.push({
              id: `fleet_${fleetID}_back_${position - 3}`,
              item: ship
            })
          }
        }
      ...
      newSubFleet (fleetID) {
        ...
        for (let position = 0; position < 3; position++) {
          ...
          for (let itemIndex = 0; itemIndex < max; itemIndex++) {
            ...
            property.pos = 'sub'
            ship.push({
              id: `${fleetID}_2_${position}_${itemIndex}`,
              property: property
            })
          }
    ...
    util: {
      ...
      editOwned (btn, type = '') {
        ...
        function editOn () {
          ...
          function updateList (target) {
            ...
            if (target) {
              target.querySelector(trans_ele).style.opacity = setOpacity(
                target.id
              )
            }
      ...
      async updateFilter (key, value, type) {
        let hasvalue = filter_setting[key].has(value),
          ...
          normal_value = (_key, _value) =>
            (hasvalue ? delete_value : add_value)(_key, _value)
        if (value == 0) {
          if (type == 'type') {
      ...
      countShipDisplayed () {
        document.querySelector('#ship_count').textContent =
          document.querySelectorAll("#shiplist button[displayed='true']")
            .length - 1
      },
      countEquipDisplayed () {
        document.querySelector('#equip_count').textContent =
          document.querySelectorAll("#equiplist button[displayed='true']")
            .length - 1
      },
      countSpWeaponDisplayed () {
        document.querySelector('#spweapon_count').textContent =
          document.querySelectorAll("#spweaponlist button[displayed='true']")
            .length - 1
      },
      equipCheck (ckid) {
        ...
        match = isCache
          ? sortedShip.find(ele => ele.id == match)
          : window[atob('c2hpcF9kYXRh')][match]
        eq = equip_data[id]
        eqck =
          filter_setting.sub.has(4 << 1) &&
          filter_setting.sub.has((128 >> 3) + 1)
            ? true
            : false
        ...
        if (ckid === atob('MjA3MDUw') || ckid === atob('MzA3MDcw')) {
          if (eqck) {
            ...
            if (isCache) {
              ...
              itemInList.icon = s2
              if (typeof itemInList.icon_cache == 'boolean')
        ...
        function restore () {
          ...
          if (isCache) {
            s1 =
              typeof itemInList.icon_cache == 'boolean'
                ? s1
                : itemInList.icon_cache
      ...
      dumpID (raw = false, input_data = [], event = undefined) {
        let data = []
        ;(!input_data.length ? fleetData : input_data).forEach(fleet =>
          data.push(app.util.dumpFleet(fleet))
        )
            ...
            if (is_empty) {
              ...
              ship.item.forEach((item, i) => {
                ...
                if (!id) {
                  ...
                  if (i == 0) {
                    if (item.property.affinity)
                      affinity = item.property.affinity
                    lv_data.push(
                      app.shipLevelLimit(lv).toString(16).padStart(2, 0)
                    )
                  }
                  if (i > 0 && i < 6) {
                    lv_data.push(
      ...
      updateFleetDataBox (json_data = '') {
        ...
        data = `${json_data}!0.07!${CryptoJS.MD5(json_data)
          .toString()
          .slice(0, 7)}`
      ...
      async loadDataByID (noDump = false, event = undefined) {
        ...
        if (!raw_data)
          raw_data = LZString.decompressFromEncodedURIComponent(
            decodeURIComponent(textbox.value)
          )
      ...
      async parseID (data, noDump = false, ver = '') {
        ...
        let default_level = [app._level_default.ship].concat(
          Array(5).fill(app._level_default.equip)
        )
        ver = parseFloat(ver)
        ...
        function set_ship ({
          ...
          app_item.affinity_value =
            app.util._affinityValueString[
              app.util._affinity_bonus[app_item.affinity]
            ]
        ...
        function set_equip ({ ship_item, app_item, level }) {
          ...
          app_item.equip_level = app.equipLevelLimit(
            app_item.rarity,
            level,
            app_item.tech
          )
        ...
        function parse007 ({
          ...
          level_data = level_data
            ...
            .flat()
          if (!(level_data instanceof Array))
            level_data = default_level.concat[10]
          for (let [item_index, id] of ship.entries()) {
            ...
            if (item_index == 6) {
              set_sp_weapon({ ship_item, app_item, level })
              continue
            }
            if (item_index == 0) {
              if (!set_ship({ ship_item, app_item, affinity_data, level }))
                is_empty_ship = true // if set ship failed, skip rest
            }
          }
        }
      ...
      extractFormation (a_fleet_data = []) {
        ...
        let formation_data = []
        a_fleet_data.forEach(fleet => {
          if (!isNaN(fleet[fleet.length - 1]))
            formation_data.push(fleet[fleet.length - 1])
        })
      ...
      setCurrent (item, noDisplay = false) {
        ...
        if (c_item === '0') {
          ...
          // show & hide filter
          if (!noDisplay) {
            lan_ship_type.forEach(
              type =>
                (type.display = use_set.has(type.id)
                  ? true
                  : type.id == 0
                  ? true
                  : false)
          ...
          if (c_item != '6') {
            ...
            let side = sideTable[c_side],
              ...
              equip_type_list =
                fleetData[c_fleet][side][c_pos].item[c_item].property.type,
              // use current ship data
              use_set = new Set(equip_type_list)
            // show & hide filter
            lan_eq_type.forEach(
              item =>
                (item.display = use_set.has(parseInt(item.id)) ? true : false)
            )
            // auto set to default
            if (
              document
                .getElementById('always_reset_equip_filter')
                .getAttribute('aria-pressed') == 'true'
            )
              app.option.equip.resetFilter(true)
            app.equipDisplay()
          } else {
            // sp weapon
            app.SpWeaponDisplay()
      ...
      dumpTechData () {
        document.querySelectorAll('#tech_data_area_expand input').forEach(e => {
      ...
      getShipReload ({
        ...
        let bonus = this._affinity_bonus[affinity] || 1,
          ...
          reload
        if (type == 20 || type == 21) tech_reload = this._tech_reload[1] || 0
        if (nationality != 97)
          strengthen = Math.floor(
            strengthen * ((Math.min(ship_level, 100) / 100) * 0.7 + 0.3)
          )

        reload = Math.floor(
          (base +
            (grow * (ship_level - 1)) / 1e3 +
            (extra * (Math.max(ship_level, 100) - 100)) / 1e3 +
            strengthen) *
            bonus +
            retrofit +
            tech_reload +
      ...
      getEquipCD ({ equip_data = {}, ship_reload = 100, pos: [f, s, p, i] }) {
        let { equip_level = 1, cd = [], eq_type, type } = equip_data,
          cd_f,
        ...
        return util.numberFormatter.dec2.format(
          round(calcCD(cd_f, ship_reload))
        )
        ...
        function calcCD (_cd, _ship_reload) {
          return (
            _cd /
            bc[k1].c1 /
            Math.sqrt((100 + _ship_reload) * (parseInt(bc[k2].c2, 16) / 100))
          )
        }
      },
      updateAllCD () {
      ...
      _eq_with_reload: new Set(
        Object.keys(equip_data)
          .filter(id => equip_data[id].eq_reload)
          .map(id => parseInt(id))
      ),
      _spw_with_reload: new Set(
        Object.keys(sp_weapon_data)
          .filter(id => sp_weapon_data[id].eq_reload.length > 0)
          .map(id => parseInt(id))
      ),
            ...
            if (index > 0) {
              setTimeout(async () => {
                ...
                if (this._airstrike.has(equip.eq_type)) {
                  ...
                  if (index != 6) {
                    equip.cd_cache = this.getEquipCD({
                      equip_data: equip,
                      ship_reload,
                      pos: [f, s, p, i]
                    })
    ...
    action: {
      ...
      getURL () {
        let data = app.util.dumpID(),
          link = LZString.decompressFromEncodedURIComponent(
            'BYFxAcGcC4HpYE4FMB2wkA8B0BzAliMAK4BGWeA9rAIIBeRCAMgIYpIBiANkkiLEA'
          )
        link = new URL(link)
      ...
      loadOwned () {
        let raw_data = LZString.decompressFromEncodedURIComponent(
          decodeURIComponent(document.getElementById('owned_data_dump').value)
        )
        if (!raw_data) msg.error.unzip_failed()
          ...
          if (hash == ck) {
            switch (version) {
              case '0.1':
                ...
                if (!((ship && equip) instanceof Array))
                  msg.error.wrong_data_type()
      ...
      loadOwnedSetting (show_msg = false) {
        let data = JSON.parse(
          LZString.decompress(LS.userSetting.get(settingKey.ownedItem))
        )
        if (data) {
    ...
    shipDisplay () {
      ...
      if (
        !document
          .getElementById('allow_dup_btn')
          .classList.contains('active') &&
        !app.util._editing_owned.ship
      )
        _hideShipInFleet()
      if (
        document.querySelector('#owned_ship_only').classList.contains('active')
      )
        _hideNotOwned()
      app.util.countShipDisplayed()
      app.getLevel('ship')
      ...
      function _isShipSelect (nation, type, rarity, retro) {
        ...
        switch (c_side) {
          case '0': // front
            if (
              filter_setting.front.has(type) ||
              filter_setting.front.size === 0
            ) {
              is_type = true
          ...
          case '1': // back
            if (
              filter_setting.back.has(type) ||
              filter_setting.back.size === 0
            ) {
              is_type = true
        ...
        if (
          filter_setting.nation.has(nation) ||
          filter_setting.nation.size === 0
        ) {
          is_nation = true
        } else if (
          filter_setting.nation.has(100) &&
          collab_nation.has(nation)
        ) {
          is_nation = true
        ...
        if (
          filter_setting.rarity.has(rarity) ||
          filter_setting.rarity.size === 0
        ) {
          is_rarity = true
        } else {
          return false
    ...
    spweaponLevelLimit (max = 10, level) {
      min = 0
    ...
    equipLevelLimit (rarity = 5, level = 13, tech = 0) {
      ...
      switch (rarity) {
        case 6:
          max = tech == 3 || tech == 0 ? 13 : 10
          break
        case 5:
          max = tech == 3 || tech == 0 ? 13 : 10
          break
        case 4:
          max = tech == 3 || tech == 0 ? 11 : 10
          break
        case 3:
          max = tech == 3 || tech == 0 ? 7 : 6
          break
        case 2:
          max = 3
          break
    ...
    getUiAffinity () {
      return parseInt(
        document.querySelector('#affinity_area .active').getAttribute('value')
      )
    },
    updateAffinity ({
      value = false,
      ...
      if (setShip) {
        ...
        if (ship) {
          if (!value) {
            ...
            if (ship.affinity > 1) {
              ship.affinity_value =
                app.util._affinityValueString[ship.affinity_value]
            }
          } else {
            // use input value
            ship.affinity = value
    ...
    getLevel (type = '', skip = false) {
      if (!skip) {
        ...
        if (type == 'equip') {
          item_in_app[`${type}_level`] = this.equipLevelLimit(
            item_in_app.rarity,
            level_app,
            item_in_app.tech
          )
    ...
    setLevel (type = '', skip = false, save = true) {
      if (!skip) {
        let side = sideTable[c_side],
          item_in_app = fleetData[c_fleet][side][c_pos].item[c_item].property,
          level_input = parseInt(
            document.getElementById(`${type}_level_input`).value,
            10
          )
        if (type == 'ship') {
        ...
        if (type == 'equip') {
          level_input = this.equipLevelLimit(
            item_in_app.rarity,
            level_input,
            item_in_app.tech
          )
    ...
    setShipAndEquip (item, save = false, skip_level = false) {
      let side = sideTable[c_side],
        ...
        shipInList = sortedShip.find(ele => {
          if (ele.id === `${item.id}` || ele.id === item.id)
        ...
        reset_equip = (_app_item, _index) => {
          Object.keys(_app_item)
            .filter(key => key != 'equip_level')
            .forEach(key => (_app_item[key] = ''))
      ...
      if (!shipInList) {
        ...
        if (match_id) {
          ...
          if (shipInList)
            console.log(
              `%cfound [${item.id}] match cn wiki [${match_id}]`,
              'color:orange'
            )
        }
      }
      ...
      if (!shipInList) {
        console.log(
          `%cship id[${item.id}] at [${pos}] not found, abort`,
          'color:red;'
        )
        return false
      }
      ...
      for (let index in shipInApp.item) {
        ...
        if (item.id === '000000') {
          ...
          if (index === '0') {
            ...
            if (skill_ship_slot[item.id])
              app_item.slot_skill = Object.assign({}, skill_ship_slot[item.id])
          } else {
            reset_equip(app_item, index)
            if (index != 6) {
              //equip
              ...
              Object.keys(app_item)
                .filter(key => key != 'equip_level')
                .forEach(key => (app_item[key] = ''))
              ...
              if (
                quantity != undefined &&
                typelist.some(eqtype => addQuantityList.has(eqtype))
              )
              ...
              ui_table.langs.forEach((lan_str, index) => {
                app_item[lan_str] = app_item[`type_${lan_str}`] =
                  type_str_arr[index].join('/')
              })
      ...
      if (!skip_level)
        app.util.updateCD({
          type: 'ship',
          data: [c_fleet, side, c_pos, c_item]
        })
      if (save) LS.userSetting.set(settingKey.fleetData, app.util.dumpID())
      return true
    },
    async equipDisplay () {
      ...
      if (
        document.querySelector('#owned_equip_only').classList.contains('active')
      )
        _hideNotOwned()
      ...
      function isEquipSelect (nation, type, rarity, tier) {
        ...
        let is_nation = false,
          ...
          is_tier = false
        if (
          filter_setting.eq_nation.has(nation) ||
          filter_setting.eq_nation.size === 0
        ) {
          is_nation = true
        } else if (
          filter_setting.eq_nation.has(100) &&
          eq_collab_nation.has(nation)
        ) {
          is_nation = true
        } else if (
          filter_setting.eq_nation.has(0) &&
          eq_other_nation.has(nation)
        ) {
          is_nation = true
        } else {
          return false
        ...
        if (
          filter_setting.eq_type.has(type) ||
          filter_setting.eq_type.size === 0
        ) {
          is_type = true
        } else {
          return false
        ...
        if (
          filter_setting.eq_rarity.has(rarity) ||
          filter_setting.eq_rarity.size === 0
        ) {
          is_rarity = true
        } else {
          return false
        ...
        if (
          filter_setting.eq_tier.has(tier) ||
          filter_setting.eq_tier.size === 0
        ) {
          is_tier = true
        } else {
          return false
    ...
    setEquip (item, save = false, skip_level = false) {
      ...
      if (!id) {
        console.log(
          `%cinvalid equip id[${item.id}] at [${pos}], abort`,
          'color:red;'
        )
        return false
      }
      ...
      if (id != 666666) {
        // copy data
        let itemInList = sortedEquip.find(ele => {
        ...
        if (!itemInList) {
          console.log(
            `%cequip id[${id}] at [${pos}] not found, abort`,
            'color:red;'
          )
          return false
        }
      ...
      if (shipInApp.item[0].property.slot_skill)
        app.setProficiencyBySkill({ c_data: [c_fleet, side, c_pos, c_item] })
    ...
    setSpWeapon (item, save = false, skip_level = false) {
      ...
      if (!id) {
        console.log(
          `%cinvalid sp weapon id[${item.id}] at [${pos}], abort`,
          'color:red;'
        )
        return false
      }
      if (id === 999999) {
        ...
        // copy data
        let itemInList = sortedSpWeapon.find(ele => {
          if (ele.id === id) return Object.assign({}, ele)
        })
        ...
        if (!itemInList) {
          console.log(
            `%csp weapon id[${id}] at [${pos}] not found, abort`,
            'color:red;'
          )
          return false
        }
    ...
    async SpWeaponDisplay () {
      ...
      sp_weapons.forEach(item => {
        ...
        if (
          type_list.includes(shiptype) &&
          (spw.limit == 0 || spw.limit == limit_id)
        ) {
          item.style.display = ''
        } else {
          item.style.display = 'none'
    ...
    checkFleetShipSkill (fleet_id) {
      ...
      if (target.length > 0)
        target.forEach(data => app.setProficiencyBySkill(data))
    },
    ...
    async initialize () {
      ...
      step('sort SP Weapon', 0)
      await createSortSpWeaponList()
      // ------------------------------
      ...
      if (indexedDB && window.idb) {
        ...
        if (no_cache_list.length > 0) {
          ...
          if (no_cache_list.length > 0) {
            ...
            console.log(
              `cached ${no_cache_list.length} icons that is missing form db`
            )
      ...
      step('load user setting', 0)
      await loadUserSetting()
      step('load fleet storage', 0)
      await loadStorage()
      step('set slider', 0)
      setSlider()
      ...
      function addTechUI () {
        ...
        data.forEach(type => {
          let ship_type = getText(type),
            ui_text = Object.keys(ship_type)
              .map(key => `ui_${key}="${ship_type[key]}"`)
              .join(' ')
        ...
        html.push(
          `<button class="btn mx-auto mb-4 line-6-item" id="tech_data_save" onclick="app.util.saveTechData()">Save</button>`
        )
      ...
      async function setBc (t) {
        let gde = n =>
            parseInt((parseInt(n) % 1e3).toString().repeat(4).slice(0, 10)),
          genC1 = n => {
            let e, t
            return (
              (t = 3 * (2 << (n - 1))),
          ...
          genC2 = n => {
            return (
              !!genC1(n << 1) &&
              '0x' +
                parseInt(`${(n << 7) - (9 << n) + ((n >> 1) << 1)}`).toString(
                  16
                )
            )
          },
        ...
        for (let i = 0; i < len; i++) {
          ...
          ;`${t}`.split('').forEach((n, i) => {
            let id = parseInt(`${i}${n.toString(16)}${t}`.slice(0, 6), 10)
              .toString(16)
              .padStart(8, 0)
            if (!list[id]) {
              hold = genC1(i)
              ...
              if (hold) {
                ...
                list[id].c2 =
                  '0x' +
                  parseInt(`${n}${i}0`).toString(16).slice(0, 3).padStart(3, 0)
              }
            }
          })
      ...
      function setSlider () {
        let ship_text = document.getElementById('ship_level_input'),
          ...
          syncText2Slider = (type = '') => {
            if (type == 'ship')
              ship_slider.value = ship_text.value = app.shipLevelLimit(
                ship_text.value
              )
            if (type == 'equip')
              equip_slider.value = equip_text.value = app.equipLevelLimit(
                5,
                equip_text.value
              )
            if (type == 'spweapon')
        ...
        spweapon_text.addEventListener('change', () =>
          syncText2Slider('spweapon')
        )

        ship_slider.addEventListener('input', () => syncSlider2Text('ship'))
        equip_slider.addEventListener('input', () => syncSlider2Text('equip'))
        spweapon_slider.addEventListener('input', () =>
          syncSlider2Text('spweapon')
        )

        ship_slider.addEventListener('change', () =>
          app.setLevel('ship', false, false)
        )
        equip_slider.addEventListener('change', () =>
          app.setLevel('equip', false, false)
        )
        spweapon_slider.addEventListener('change', () =>
          app.setLevel('spweapon', false, false)
        )
      ...
      async function addClickEventAndImg () {
        ...
        let list = [
          ...
          count = 0,
          bp = ['MTA4MDIw', 'MjgzNDA='].reduce(
            (a, b) => (a.push(parseInt(atob(b))), a),
            []
          )
      ...
      function createNewItem (data, progress) {
        let html = `
                    <button class="p-1 item_container" data-dismiss="modal" displayed="true" style="opacity: 1;" id="${
                      data.id
                    }">
                        <div class="container-fluid p-0 box">
                            <div class="container-fluid icon_box">
                                <img class="img-fluid bg" src="${data.bg}">
                                <img class="img-fluid frame" src="${
                                  data.frame
                                }">
                                <img class="img-fluid icon" src="${
                                  ui_table.empty_disable
                                }">
                            </div>
                            <span class="item_name" name="name" tw="${htmlesc(
                              data.tw
                            )}" cn="${htmlesc(data.cn)}" en="${htmlesc(
          data.en
        )}" jp="${htmlesc(data.jp)}">
      ...
      async function createAllSpWeapon () {
        ...
        await addProgressBar(
          'create_spweapon',
          'Generate SP Weapons',
          sortedSpWeapon.length,
          _loading_.spweapon
        )
        let pos = document.querySelector('#spweaponlist'),
          html = sortedSpWeapon.map(item =>
            createNewItem(item, _loading_.spweapon)
          )
        setTimeout(() => {
          pos.innerHTML = html.join('')
      ...
      async function getMissingCache (no_cache_list) {
        ...
        await addProgressBar(
          'fetch_missing',
          'Found some icons have no cache, download it now...',
          no_cache_list.length,
          progress
        )
      ...
      async function imgToDataURI () {
        ...
        await addProgressBar(
          'fetch_img',
          'Downloading images... This will take a while.',
          count,
          progress
        )
      ...
      async function loadImgCache (AFDB) {
        ...
        let reg = /.*(?:equips|shipicon|spweapon)\/([^\.]+).*/,
          ...
          count = 0
        all_cache = all_cache.reduce(
          (obj, cache_data) => (
            (obj[cache_data.src] = cache_data.data_url), obj
          ),
          {}
        ) // convert to obj
      ...
      async function saveCacheData (db, db_name, cacheData) {
        ...
        const promise_list = cacheData.map(obj => {
          return tx.store.add(obj)
        })
      ...
      function splitButtonGroup (
        ...
        if (max_per_line != 5 && alt_class.length == 0)
          throw Error('max != 5, but no alt_class.')
        ...
        buttons.forEach((btn, index) => {
          if (index % max_per_line == 0) {
            ...
            new_line.className =
              'btn-group d-flex flex-wrap ml-1' +
              (line_count > 0 ? ' mt-1' : '')
      ...
      async function loadUserSetting () {
        ...
        if (setting[settingKey.techData]) {
          // load tech before load fleet
        ...
        ;[
          ...
          if (target.id != 'display_fleet_op') {
            ...
            setTimeout(() =>
              setValue(target.key, target.id, () => app.option._edit_op_css(1))
            )
          }
        })
        ...
        if (setting[settingKey.compactMode] >= 1) {
          setTimeout(() =>
            document
              .getElementById(`compact_mode${setting[settingKey.compactMode]}`)
              .click()
          )
        }
  ...
  dynamicFleet = {
    ...
    deleteFleet (ele) {
      let del_pos = this.getPos(ele),
        ...
        new_fleet = []
      current_fleet_dump.forEach((fleet, index) => {
        if (index != del_pos) new_fleet.push(fleet)
      })
    ...
    insertFleet (ele) {
      ...
      current_fleet_dump.forEach((fleet, index) => {
        if (direction == 0 && insert_position == index)
          new_fleet.push(new_insert_fleet)
        new_fleet.push(fleet)
        if (direction == 1 && insert_position == index)
          new_fleet.push(new_insert_fleet)
      })
      new_fleet = JSON.stringify(new_fleet)
    ...
    disableInvalidMoveButton () {
      let all = document.querySelectorAll(
          `[onclick^="dynamicFleet.moveFleet"],[onclick^="dynamicFleet.deleteFleet"]`
        ),
      ...
      all = document.querySelectorAll(
        `[onclick^="dynamicFleet.copyFleet"],[onclick^="dynamicFleet.insertFleet"]`
      )
    ...
    async swapPos (btn, copy_ship = false) {
      let target_list = document.querySelectorAll(
          [
            '#options',
            '.fleet_op_box',
            '#empty_fleet',
            ...
            if (!_elem.ELEMENT_NODE) {
              ...
              while (__count <= _max_rase && !__find && __up_layer) {
                if (__up_layer.getAttribute(_attr) == _value) {
                  ...
                  let children = [...__up_layer.children].find(
                    e => e.getAttribute(_attr) == _value
                  )
      ...
      async function copyShip () {
        ...
        a.forEach((item, item_index) => {
          ...
          b[item_index].property = JSON.parse(
            JSON.stringify(a[item_index].property)
          )
        })
      ...
      async function swapShip () {
        ...
        a.forEach((item, item_index) => {
          ;[a[item_index].property, b[item_index].property] = [
            b[item_index].property,
      ...
      function disableOtherSide (ele, index) {
        if (index == 0)
          if (ele.name.split('_')[1] != sw.a[1]) disableElement(ele)
      }
      ...
      function disableToggle (ele) {
        if (ele.getAttribute('data-toggle'))
          ele.setAttribute('data-toggle', 'false')
      }
...
const maximumFleet = 30,
  ...
  // ship
  type_front = new Set([1, 2, 3, 18, 19, 20, 21, 23]),
  type_back = new Set([4, 5, 6, 7, 10, 12, 13, 20, 21, 24]),
  ...
  other_front = new Set([19, 20, 21, 23]),
  other_back = new Set([10, 20, 21, 24]),
...
const fleet_btn_style = {
  ...
  path = (target = '') => {
    return `dynamicFleet.${target}(this)`
  },
  action = {
...
Vue.component('fleet-container', {
  ...
  template: `
        <div v-bind:class="class_data.fleet_box_o">
            <div class="fleet_op_box" v-if="ui_settings.show_op">
                <div class="d-flex line-4-item">
                    ...
                    <button class="${
                      fleet_btn_style.copy
                    }" v-bind:pos="fleet.id" onclick="${action.swap_copy.replace(
    'this',
    'this, true'
  )}" v-text="ui_text.copy_ship[lang]">CopyShip</button>
                </div>
                <div class="d-flex line-6-item">
                    <div class="d-flex btn-group w-100 m-auto">
                        <button class="${
                          fleet_btn_style.yellow
                        }" v-bind:pos="fleet.id" data="1,1" onclick="${
    action.insert
  }" v-text="'+'+ui_text.normal_fleet[lang]">Normal</button>
                    </div>
                </div>
                <div class="d-flex line-3-item">
                    <div class="d-flex btn-group w-100 mx-1 my-auto">
                        <button class="${
                          fleet_btn_style.normal
                        } line-3-item w-border-right" v-bind:pos="fleet.id" onclick="${
    action.move
  }" data="-1">▲</button>
                        <div class="${
                          fleet_btn_style.normal
    ...
    action.swap
  }" v-text="ui_text.swap_ship[lang]">Swap</div>
                        <button class="${
                          fleet_btn_style.normal
                        } line-3-item border-left" v-bind:pos="fleet.id" onclick="${
    action.move
  }" data="1">▼</button>
                    </div>
                </div>
                <div class="d-flex line-6-item">
                    <div class="d-flex btn-group w-100 m-auto">
                        <button class="${
                          fleet_btn_style.yellow
                        }" v-bind:pos="fleet.id" data="2,1" onclick="${
    action.insert
  }" v-text="'+'+ui_text.sub_fleet[lang]">Sub</button>
                    </div>
                </div>
                <div class="d-flex line-4-item">
                    <button class="${
                      fleet_btn_style.copy
                    }" v-bind:pos="fleet.id" onclick="${
    action.copy
  }" v-text="ui_text.copy_fleet[lang]">CopyFleet</button>
                    <button class="${
                      fleet_btn_style.del
                    }" v-bind:pos="fleet.id" onclick="${
    action.delete
...
const filter_btn_class =
    'btn btn-outline-secondary line-5-item py-2 ml-0 font-weight-bold text-truncate filter-btn',
  filter_btn_template = `<button type="button" onclick="app.util.updateFilterSetting(this)" class="${filter_btn_class}"></button>`,
  filter_btn_template_4item = filter_btn_template.replace(
    'line-5-item',
    'line-4-item'
  ),
  filter_btn_template_6item = filter_btn_template.replace(
    'line-5-item',
    'line-6-item'
  )
...
Vue.component('ship-nation-button', {
  props: ['nation', 'lang'],
  template: filter_btn_template_6item
})
Vue.component('ship-type-button', {
  props: ['type', 'lang'],
  template: filter_btn_template
})
...
//----------------------------------------------------------
Vue.component('equip-nation-button', {
  props: ['nation', 'lang'],
  template: filter_btn_template_6item
})
Vue.component('equip-type-button', {
  props: ['type', 'lang'],
  template: filter_btn_template
})
Vue.component('equip-rarity-button', {
  props: ['rarity', 'lang'],
  template: filter_btn_template
})
Vue.component('equip-tier-button', {
  props: ['tier', 'lang'],
  template: filter_btn_template_4item
})
